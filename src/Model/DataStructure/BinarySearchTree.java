...
public class BinarySearchTree<ContentType extends ComparableContent<ContentType>> {
		...
		if(pContent != null){
			if(isEmpty()){
				...
				if(getRightTree().isEmpty()){
					...
					this.getRightTree().insert(pContent);
				}
			}else if (pContent.isLess(node.content)){
				if(getLeftTree().isEmpty()){
					getLeftTree().node = new BSTNode<>(pContent);
				}else{
					this.getLeftTree().insert(pContent);
				}
			}
		}
	}
	...
	public ContentType search(ContentType pContent) {
		if(pContent != null){
			...
			}else if(pContent.isLess(getContent()) && !this.getLeftTree().isEmpty()){
